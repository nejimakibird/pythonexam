...
fName = ".\92800_1804180002.txt"

# ファイルオープン
fobj = open(fName,mode='rb',buffering=-1,encoding=None,errors=None)
py_datas = []
record_format1 = '2s5s9s2s1s2s3s8s8s8s15s3s2s18s19s20s50s80s30s40s1s20s1s1s9s9s9s9s15s15s4s4s6s20s6s20s9s1s20s1s2s2s4s5s5s2s1s1s1s10s9s9s7s7s7s10s2s1s2s2s2s5s5s5s9s11s9s9s15s20s20s10s5s1s1s1s2s2s2s1s1s1s2s2s2s40s60s2s2s2s8s6s2s2s2s1s1s1s5s5s5s2s117s2s'
...
try:
    while True:
        ...
        if py_data[0] == b'H1':
            ...
            # ヘッダ２レコード
            py_data = struct.unpack(record_format2,buf)
            
        elif py_data[0] == b'H3':
            # ヘッダ３レコード
            py_data = struct.unpack(record_format3,buf)

        elif py_data[0] == b'BD':
            # ボディレコード
            py_data = struct.unpack(record_formatBD,buf)
    ...
    with open('some.csv', 'w') as f:
        ...
        writer.writerows(py_csv) # 2次元配列も書き込める
