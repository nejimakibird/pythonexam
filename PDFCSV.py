...
with open(sys.argv[1], 'rb') as f:
    ...
    for page in PDFPage.get_pages(f):
        ...
        boxes.sort(key=lambda b: (int(-b.y1), int(b.x0)))

        for box in boxes:
            print_and_write('-' * 10)  # 読みやすいよう区切り線を表示する。
            print_and_write(box.get_text().replace(' ','').replace('\n',' ').strip() + '\t' + str(box.y1) + ',' + str(box.x0))  # テキストボックス内のテキストを表示する。
